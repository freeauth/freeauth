...
public:
  ...
  FpreFerretCOT(in_io_type _io, int in_party, int bsize = 1000, int tag = 0) {
    ...
    if (party == ALICE) {
      ...
      abit1->setup(Delta, string("/tmp/abit1") + std::to_string(party) + std::to_string(tag));
      abit2->setup(string("/tmp/abit2") + std::to_string(party) + std::to_string(tag));
      Delta = abit1->Delta;
    } else {
      Delta = Delta ^ makeBlock(0x0, 0x1);
      abit1->setup(string("/tmp/abit1") + std::to_string(party) + std::to_string(tag));
      abit2->setup(Delta, string("/tmp/abit2") + std::to_string(party) + std::to_string(tag));
      Delta = abit2->Delta;
    }
