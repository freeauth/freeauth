...
Server::Server(bssl::UniquePtr<SSL_CTX> &&ctx, const std::string &ip_address,
               const bool is_ip_v6, const int backlog, const uint16_t port) noexcept
    : ssl_ctx{std::move(ctx)}, key_shares{}, public_key{},
      ...
      handshake_key_shares{}, version{}, cipher_suite{}, timer{}, should_exit{false},
      bandwidth_tracker{} {
  ...
  [[maybe_unused]] const bool setup = [&]() {
    if (is_ip_v6) {
      return socket.set_ip_v6() && socket.set_port(port) && socket.set_addr(ip_address) &&
             socket.bind() && socket.listen(backlog);
    }

    return socket.set_ip_v4() && socket.set_port(port) && socket.set_addr(ip_address) && socket.bind() &&
...
bool Server::accept() noexcept {
  ...
  for(size_t i = 0; i<CIRCUIT_NUM; i++) {
    ...
    if(socket_circuits[i] == nullptr) return false;
...
void preproc_circuts_thread(EmpWrapperAG2PC *circuit, const char *name, BandwidthTracker::TrackerType &bandwidth_tracker) {
  if(!circuit) return;
  circuit->do_preproc();
  TRACK_IF_INTERESTED(Events::State::CIRCUIT_PREPROC,
                        circuit->get_counter());
  printf("[Verifier] Preproc %s circuit... done\n", name);
}
...
bool Server::do_preproc(const bool should_preproc) noexcept {
  ...
  auto start = std::chrono::steady_clock::now();
    traffic_circuit.reset(EmpWrapperAG2PC::build_derive_ts_circuit(
      socket_circuits[0]->get_ssl_object(), emp::BOB,
      EmpWrapperAG2PCConstants::TRAFFIC_CIRCUIT_TAG));
  if (traffic_circuit) {
    circuits_preproc_threads.emplace_back(std::thread(preproc_circuts_thread, traffic_circuit.get(), "traffic", std::ref(bandwidth_tracker)));
  }

  aes_joint_circuit.reset(EmpWrapperAG2PC::build_joint_aes_circuit(
      socket_circuits[1]->get_ssl_object(), emp::ALICE,
      EmpWrapperAG2PCConstants::AES_JOINT_CIRCUIT_TAG));
  // if (aes_joint_circuit) {
  // }

  gcm_circuit.reset(EmpWrapperAG2PC::build_gcm_circuit(
      socket_circuits[2]->get_ssl_object(), emp::BOB,
      EmpWrapperAG2PCConstants::GCM_CIRCUIT_TAG));
  if (gcm_circuit) {
    circuits_preproc_threads.emplace_back(std::thread(preproc_circuts_thread, gcm_circuit.get(), "gcm share", std::ref(bandwidth_tracker)));
  }

  gcm_tag_circuit.reset(EmpWrapperAG2PC::build_gcm_tag_circuit(
      socket_circuits[3]->get_ssl_object(), emp::BOB,
      EmpWrapperAG2PCConstants::GCM_MAKE_CIRCUIT_TAG));
  // if (gcm_tag_circuit) {
  // }

  gcm_verify_circuit.reset(EmpWrapperAG2PC::build_gcm_vfy_circuit(
      socket_circuits[4]->get_ssl_object(), emp::BOB,
      EmpWrapperAG2PCConstants::GCM_VERIFY_CIRCUIT_TAG));
  // if (gcm_verify_circuit) {
  // }
  ...
  handshake_circuits[0].reset(EmpWrapperAG2PC::build_derive_hs_circuit(
      socket_circuits[5]->get_ssl_object(), SSL_CURVE_SECP256R1, emp::BOB,
      EmpWrapperAG2PCConstants::HANDSHAKE_CIRCUIT_TAG_A));
  if (handshake_circuits[0]) {
    circuits_preproc_threads.emplace_back(std::thread(preproc_circuts_thread, handshake_circuits[0].get(), "HS1", std::ref(bandwidth_tracker)));
  }

  handshake_circuits[1].reset(EmpWrapperAG2PC::build_derive_hs_circuit(
      socket_circuits[6]->get_ssl_object(), 0, emp::BOB, // can be SSL_CURVE_SECP384R1
      EmpWrapperAG2PCConstants::HANDSHAKE_CIRCUIT_TAG_B));

  if (handshake_circuits[1]) {
    circuits_preproc_threads.emplace_back(std::thread(preproc_circuts_thread, handshake_circuits[1].get(), "HS2", std::ref(bandwidth_tracker)));
  }
  for(auto &i : circuits_preproc_threads) {
    if(i.joinable()) i.join();
  }
  auto tend = std::chrono::steady_clock::now();
  auto duration = std::chrono::duration_cast<std::chrono::duration<double>>(tend - start);
  printf("[Verifier] Preproc all circuits cost %fs\n", duration.count());
  circuits_preproc_threads.clear();
...
bool read_single_header(TLSSocket &socket,
  ...
  auto func = [](CBS &cbs, uint8_t &arg) {
    CBS_get_u8(&cbs, &arg);
    return Messaging::is_valid_header(arg);
  };
...
bool Server::read_transcript_data(std::vector<uint8_t> &recv_store,
  ...
  // Reserve the size up.
  recv_store.reserve(recv_store.size() + size);
  ...
  do {
    ...
    // allocations.
    recv_store.insert(recv_store.end(), buffer.begin(), buffer.begin() + read);
  } while (socket.pending() != 0);

  return true;
}

bool Server::read_chts_shts_commit() noexcept {
  ...
  if (!read_transcript_data(transcript,
    ...
    PRINT_IF_LOUD("Failed to read data in read_chts_shts_commit");
    return false;
  }
  ...
  if (transcript.size() < hash_size) {
    PRINT_IF_LOUD("Transcript size too small in read_chts_shts_commit");
    return false;
  }

  memcpy(chts_shts_comm.data(), transcript.data(), hash_size);
...
bool Server::send_shts_chts() noexcept {
...
bool Server::read_h3h4() noexcept {
  // Read in the hash so far.
  if (!read_transcript_data(h3h4, Messaging::MessageHeaders::H6_SEND)) {
    PRINT_IF_LOUD("Failed to read h3 h4 in read_h3h4");
    return false;
  }
  this->state = next_state(this->state);
  return true;
...
bool Server::derive_ts() noexcept {
  ...
  get_ssl()->server_key_share = traffic_key_shares.server_key_share;
  get_ssl()->client_key_share = traffic_key_shares.client_key_share;
  // Note: the +4s here are because the AES IV is 96 bits to begin with,
  // but we later expand this to 128 bits.
  std::copy(traffic_key_shares.client_iv.begin(), traffic_key_shares.client_iv.end(),
            get_ssl()->client_iv.begin());
  std::copy(traffic_key_shares.server_iv.begin(), traffic_key_shares.server_iv.end(),
            get_ssl()->server_iv.begin());
...
bool run_gcm_make_tag_circuit(EmpWrapperAG2PC *const circuit,
...
bool Server::attest() noexcept {
  Messaging::MessageHeaders header;
  bool worked = true;
  EmpWrapperAG2PC::preproc_aes_gcm_circuits(
  ...
  */
  ...
  while (!should_stop(this->state, stop_state)) {
    ...
    case ServerState::VERIFY_SERVER_CERT:
      PRINT_IF_LOUD("Verify [ServerCertificate,ServerCertificateVerify,ServerFinished]");
