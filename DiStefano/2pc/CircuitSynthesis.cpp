...
std::string project_root_relative_path;
...
static void sha256_internal(const emp::block *const input,
  ...
  // n-for-1-auth.
  auto circuit_filepath = project_root_relative_path + CircuitSynthesis::SHA256_FILEPATH;
...
static void add_two_mod_p_internal(const bool *const a, const bool *const b,
  ...
  auto get_circuit_path = []() {
    if constexpr (length == 256) {
      return project_root_relative_path + CircuitSynthesis::MA_256_FILEPATH;
    } else if constexpr (length == 384) {
      return project_root_relative_path + CircuitSynthesis::MA_384_FILEPATH;
    } else if constexpr (length == 521) {
      return project_root_relative_path + CircuitSynthesis::MA_521_FILEPATH;
    }
  ...
  static std::string file_path = get_circuit_path();
  // We'll make sure the file actually exists (in debug mode only!)
  assert_valid_filepath(file_path.c_str());
  ...
  // above.
  emp::BristolFashion bf(file_path.c_str());
