...
static void DeriveAESEncryption() {
  ...
  emp::block out[128];
  auto circuit_filepath =
      project_root_relative_path + CircuitSynthesis::AES128_FULL_FILEPATH;
  assert_valid_filepath(circuit_filepath.c_str());
  emp::BristolFormat bf(circuit_filepath.c_str());
...
template <unsigned int length> static void ProduceCombinedCircuit() noexcept {
  ...
  constexpr auto mask_bits = 3 * 128;
  ...
  std::string true_file_path = project_root_relative_path + file_path;
  assert_valid_filepath(true_file_path.c_str());

  emp::BristolFashion bf(true_file_path.c_str());
...
static void DeriveAESSplit() noexcept {
  ...
  std::string circuit_filepath =
      project_root_relative_path + CircuitSynthesis::AES128_FULL_FILEPATH;
  assert(Util::is_valid_filepath(circuit_filepath.c_str()));
  emp::BristolFormat bf(circuit_filepath.c_str());
...
static void DeriveCommitments_128() {

  emp::setup_plain_prot(true, "derive_commitments_sha256_128.txt");

  // This circuit derives commitments to a particular AES CTR mode "block key".

  // INPUT:iv+key+pt+random
  // OUTPUT:was_same+sha256+ct
  ...
  memcpy(key_and_iv + 128, alice_blocks, sizeof(emp::block) * 128);
  ...
  for (unsigned i = 0, j = 0, k = 0; i < 128; i++, j++) {
    if (j == 8) {
      j = 0;
      k += 8;
    }
    output[i] = emp::CircuitExecution::circ_exec->xor_gate(
        output[i], alice_blocks[256 + 7 - j + k]);
  ...
  // do sha256
  emp::block output_sha256[256];
  EmpBlockOwningSpan input_to_hash_function(256);
  std::copy(alice_blocks + 256, alice_blocks + 512,
            input_to_hash_function.data());
  CircuitSynthesis::sha256(input_to_hash_function, output_sha256);

  bool output_b[128];
  bool output_same[8];
  bool output_hash[256];
  ...
  emp::ProtocolExecution::prot_exec->reveal(output_same, emp::PUBLIC, same_arr,
                                            8);
  emp::ProtocolExecution::prot_exec->reveal(output_hash, emp::PUBLIC,
                                            output_sha256, 256);
  emp::ProtocolExecution::prot_exec->reveal(output_b, emp::PUBLIC, output, 128);
  emp::finalize_plain_prot();
}

template <unsigned BLOCK_NUM> static void DeriveCommitments() {

  emp::setup_plain_prot(true, "derive_commitments_sha256_" +
                                  std::to_string(BLOCK_NUM * 128) + ".txt");
  ...
  constexpr auto input_size = 128 + 128 + 128 * BLOCK_NUM + 128;
  bool alice_input[input_size];
  bool bob_input[input_size];
  ...
  // For now just take Alice's IV.
  emp::block IV[128];
  memcpy(IV, alice_blocks, sizeof(emp::block) * 128);
  // Now we can do the full encryption.
  // The full AES circuit expects key then ciphertext in a single block.
  std::string aes_circuit_filepath =
      project_root_relative_path + CircuitSynthesis::AES128_FULL_FILEPATH;
  assert(Util::is_valid_filepath(aes_circuit_filepath.c_str()));
  emp::BristolFormat bf(aes_circuit_filepath.c_str());

  std::string inc_circuit_file =
      project_root_relative_path + "DiStefano/2pc/vhdl/Inc128.txt";
  assert(Util::is_valid_filepath(inc_circuit_file.c_str()));
  emp::BristolFashion inc(inc_circuit_file.c_str());

  emp::block output_one_block[128];
  emp::block output[128 * BLOCK_NUM];
  // Check the IVs are the same.
  const auto were_same = equals<128>(alice_blocks, bob_blocks);

  for (unsigned block_index = 0; block_index < BLOCK_NUM; block_index++) {
    memcpy(key_and_iv + 128, IV, sizeof(IV));
    unsigned block_start = block_index * 128;
    bf.compute(output_one_block, key_and_iv, key_and_iv);
    for (unsigned i = 0, j = 0, k = 0; i < 128; i++, j++) {
      if (j == 8) {
        j = 0;
        k += 8;
      }
      output[block_start + i] = emp::CircuitExecution::circ_exec->xor_gate(
          output_one_block[i], alice_blocks[256 + block_start + 7 - j + k]);
      // Mask against the output.
      output[block_start + i] = emp::CircuitExecution::circ_exec->and_gate(
          output[i + block_start], were_same);
    }

    inc.compute(output_one_block, IV);
    memcpy(IV, output_one_block, sizeof(IV));
  }

  // do sha256
  emp::block output_sha256[256];
  EmpBlockOwningSpan input_to_hash_function((BLOCK_NUM + 1) *
                                            128); // Random + PT
  std::copy(alice_blocks + 256, alice_blocks + input_size,
            input_to_hash_function.data());
  CircuitSynthesis::sha256(input_to_hash_function, output_sha256);

  bool output_b[128 * BLOCK_NUM];
  bool output_same[8];
  bool output_hash[256];
  ...
  emp::ProtocolExecution::prot_exec->reveal(output_hash, emp::PUBLIC,
                                            output_sha256, 256);
  emp::ProtocolExecution::prot_exec->reveal(output_b, emp::PUBLIC, output,
                                            128 * BLOCK_NUM);
  emp::finalize_plain_prot();
}
...
template <unsigned size> static void DeriveAESEnc() {
  ...
  std::string circuit_filepath =
      project_root_relative_path + CircuitSynthesis::AES128_FULL_FILEPATH;
  assert(Util::is_valid_filepath(circuit_filepath.c_str()));
  emp::BristolFormat aes(circuit_filepath.c_str());

  std::string circuit_file =
      project_root_relative_path + "DiStefano/2pc/vhdl/Inc128.txt";
  assert(Util::is_valid_filepath(circuit_file.c_str()));
  emp::BristolFashion inc(circuit_file.c_str());
...
int main(int, char **argv) {
  project_root_relative_path = std::string(dirname(argv[0])) + "/../../";
  DeriveCombined256();
  // DeriveHS384();
  // DeriveHS521();
  // DeriveCHTSATS();
