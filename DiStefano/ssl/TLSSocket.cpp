...
bool TLSSocket::set_verifier_connection(SSL *const server_ssl) noexcept {
  if (is_server()) {
    return false;
  }

  server_ssl->verifier = this->ssl;
  server_ssl->socket_circuits = new SSL*[CIRCUIT_NUM];
  for(size_t i = 0;i < CIRCUIT_NUM; i++) {
    server_ssl->socket_circuits[i] = socket_circuits[i]->get_ssl_object();
  }
  return true;
}
...
TLSSocket::TLSSocket(SSL_CTX &ssl_ctx, const bool is_server)
    : StatefulSocket(is_server), ssl{SSL_new(&ssl_ctx)}, ssl_ctx(&ssl_ctx) {}
...
std::unique_ptr<TLSSocket> TLSSocket::accept_tlsconnection() noexcept {
  // The only difference between this function and StatefulSocket's accept
  // is that we transfer the BIO ownership over to the SSL object here.
  // std::unique_ptr<StatefulSocket>  psocket = StatefulSocket::accept_connection();
  std::unique_ptr<TLSSocket> tls_socket_p(new TLSSocket(ssl_ctx, true));
  tls_socket_p->sock = sock;
  tls_socket_p->bound = bound;
  tls_socket_p->listening = listening;
  if(tls_socket_p->accept()) {
    return  tls_socket_p;
  }
  return nullptr;
}
...
bool TLSSocket::set_send_handshake_data_callback(SSL::send_handshake_data_function_type cb) noexcept {
  if(is_server()) {
    return false;
  }
  return SSL::set_send_handshake_data(ssl, cb);
}

bool TLSSocket::set_aes_encrypt(
  SSL::aes_encrypt_function_type cb) noexcept {
    if(is_server()) {
      return false;
    }

    return SSL::set_aes_encrypt(ssl, cb);
}

bool TLSSocket::set_aes_decrypt(
  SSL::aes_decrypt_function_type cb) noexcept {
    if(is_server()) {
      return false;
    }

    return SSL::set_aes_decrypt(ssl, cb);
}
