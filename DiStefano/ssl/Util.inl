...
void Util::swap_bitorder(InputIterator in_it, OutputIterator out_it, size_t length) {
  static_assert(sizeof(*in_it) == sizeof(*out_it) && sizeof(*in_it) == 1);
  for(size_t i = 0; i < length; in_it++, out_it++, i++) {
    uint8_t value = 0, tmp = *in_it;
    for(int j = 7;j >= 0;j--) {
      value += static_cast<uint8_t>((tmp & 1) << j);
      tmp >>= 1;
    }
    *out_it = value;
  }
...
template <class InputIterator, class OutputIterator>
void Util::swap_byte_order(InputIterator in_it, OutputIterator out_it, size_t length) {
  static_assert(sizeof(*in_it) == sizeof(*out_it) && sizeof(*in_it) == 1);
  out_it += length - 1;
  for(size_t i = 0; i < length; in_it++, out_it--, i++) {
    *out_it = *in_it;
...
template <class InputIterator, class OutputIterator>
void Util::xor_func(InputIterator in_it, OutputIterator out_it, size_t length) {
  static_assert(sizeof(*in_it) == sizeof(*out_it) && sizeof(*in_it) == 1);
  for(size_t i = 0; i < length; in_it++, out_it++, i++) {
    *out_it ^= *in_it;
  }
...
template <class T>
void Util::print_hex_data(const T &data, const char *prompt) noexcept {
