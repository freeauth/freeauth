...
EmpWrapperAG2PC::EmpWrapperAG2PC(SSL *const ssl,
    ...
    : wrapper{ssl}, addr{&wrapper, &wrapper},
      ...
      circuitname(strrchr(filepath, '/') + 1) {}
...
bool EmpWrapperAG2PC::exec_small_internal(
  ...
  */
  ...
  auto begin_online = std::chrono::steady_clock::now();
  switch (type) {
  case CircuitType::SINGLE:
...
void EmpWrapperAG2PC::preproc_aes_gcm_circuits(
    EmpWrapperAG2PC *tag_circuit, EmpWrapperAG2PC *vfy_circuit,
    EmpWrapperAG2PC *aes_circuit, bool single) noexcept {
  auto start = std::chrono::steady_clock::now();
  tag_circuit->gcm_tag.reset(new GCMTagCircuitType(
      tag_circuit->addr, tag_circuit->party,
      EmpWrapperAG2PCConstants::GCM_MAKE_CIRCUIT_TAG, &(tag_circuit->circuit)));
  // vfy_circuit->gcm_vfy.reset(
  //     new GCMVfyCircuitType(vfy_circuit->addr, vfy_circuit->party,
  //                           EmpWrapperAG2PCConstants::GCM_VERIFY_CIRCUIT_TAG,
  //                           &(vfy_circuit->circuit)));
  aes_circuit->aes.reset(
      new AESCircuitType(aes_circuit->addr, aes_circuit->party,
                         EmpWrapperAG2PCConstants::AES_JOINT_CIRCUIT_TAG,
                         &(aes_circuit->circuit)));
  std::thread t1;
  if(single) {
    tag_circuit->do_preproc();
    // vfy_circuit->do_preproc();
  } else {
    t1 = std::thread(&EmpWrapperAG2PC::do_preproc, tag_circuit);
    // t2 = std::thread(&EmpWrapperAG2PC::do_preproc, vfy_circuit);
  }
  aes_circuit->do_preproc();
  if(!single) {
    t1.join();
    // t2.join();
  }
  ...
  if(tag_circuit->party == emp::BOB)
    printf("[%s] Preproc aes gcm_tag gcm_cfy cost %fs\n", "Verifier", duration.count());
}
...
bool EmpWrapperAG2PC::make_tag(
  ...
  if (run_times == tag_iters) {
    if(vfy_circuit->run_times == vfy_iters) {
      preproc_aes_gcm_circuits(this, vfy_circuit, aes_circuit);
    } else {
      gcm_tag.reset(new GCMTagCircuitType(addr, party, EmpWrapperAG2PCConstants::GCM_MAKE_CIRCUIT_TAG, &circuit));
      do_preproc();
    }
  }
...
bool EmpWrapperAG2PC::verify_tag(
  ...
  if (run_times == vfy_iters) {
    if (tag_circuit->run_times == tag_iters) {
      preproc_aes_gcm_circuits(tag_circuit, this, aes_circuit);
    } else {
      gcm_vfy.reset(new GCMVfyCircuitType(
          addr, party, EmpWrapperAG2PCConstants::GCM_VERIFY_CIRCUIT_TAG,
          &circuit));
      do_preproc();
    }
  }
...
bool EmpWrapperAG2PC::do_joint_aes(
  ...
  if (run_times == EmpWrapperAG2PC::aes_iters) {
    aes.reset(new AESCircuitType(addr, party, EmpWrapperAG2PCConstants::AES_JOINT_CIRCUIT_TAG, &circuit));
    do_preproc();
  }
...
bool EmpWrapperAG2PC::commit_data(
...
EmpWrapperAG2PC::build_gcm_vfy_circuit(SSL *const ssl, const int mode,
  ...
  res->run_times = vfy_iters;
  return res;
}

EmpWrapperAG2PC *
EmpWrapperAG2PC::build_commit_circuit(SSL *const ssl, const int mode,
                                       const int tag) noexcept {
  auto res =
      new EmpWrapperAG2PC(ssl, EmpWrapperAG2PCConstants::COMMIT_FILEPATH);
  res->set_twopc(mode, tag);
  return res;
}
...
void EmpWrapperAG2PC::do_preproc() noexcept {
  auto begin_preproc = std::chrono::steady_clock::now();
  do_preproc_indep();
  do_preproc_dep();
