...
bool ThreePartyHandshake::preproc_circuits_before_handshake(SSL *ssl, bool single) noexcept {
  std::vector<std::thread> circuits_preproc_threads;

  ssl->traffic_circuit = EmpWrapperAG2PC::build_derive_ts_circuit(
      ssl->socket_circuits[0], emp::ALICE, EmpWrapperAG2PCConstants::TRAFFIC_CIRCUIT_TAG);
  if (ssl->traffic_circuit) {
    if(single) {
      ssl->traffic_circuit->do_preproc();
    }
    else
      circuits_preproc_threads.emplace_back(std::thread(&EmpWrapperAG2PC::do_preproc, ssl->traffic_circuit));
  }

  ssl->aes_joint_circuit = EmpWrapperAG2PC::build_joint_aes_circuit(
      ssl->socket_circuits[1], emp::BOB, EmpWrapperAG2PCConstants::AES_JOINT_CIRCUIT_TAG);
  // if (ssl->aes_joint_circuit) {
  // }

  ssl->gcm_circuit = EmpWrapperAG2PC::build_gcm_circuit(
      ssl->socket_circuits[2], emp::ALICE, EmpWrapperAG2PCConstants::GCM_CIRCUIT_TAG);
  if (ssl->gcm_circuit) {
    if(single) {
      ssl->gcm_circuit->do_preproc();
    }
    else
      circuits_preproc_threads.emplace_back(std::thread(&EmpWrapperAG2PC::do_preproc, ssl->gcm_circuit));
  }

  ssl->make_tag_circuit = EmpWrapperAG2PC::build_gcm_tag_circuit(
      ssl->socket_circuits[3], emp::ALICE, EmpWrapperAG2PCConstants::GCM_MAKE_CIRCUIT_TAG);
  // if (ssl->make_tag_circuit) {
  // }

  ssl->verify_tag_circuit = EmpWrapperAG2PC::build_gcm_vfy_circuit(
      ssl->socket_circuits[4], emp::ALICE, EmpWrapperAG2PCConstants::GCM_VERIFY_CIRCUIT_TAG);
  // if (ssl->verify_tag_circuit) {
  // }
    ...
    ssl->handshake_circuit_a = EmpWrapperAG2PC::build_derive_hs_circuit(
        ssl->socket_circuits[5], SSL_CURVE_SECP256R1, emp::ALICE,
    ...
    if (ssl->handshake_circuit_a) {
      if(single) {
        ...
        circuits_preproc_threads.emplace_back(std::thread(&EmpWrapperAG2PC::do_preproc, ssl->handshake_circuit_a));
    }

    ssl->handshake_circuit_b = EmpWrapperAG2PC::build_derive_hs_circuit(
        ssl->socket_circuits[6], 0, emp::ALICE, // it can be SSL_CURVE_SECP384R1
        EmpWrapperAG2PCConstants::HANDSHAKE_CIRCUIT_TAG_B);
    if (ssl->handshake_circuit_b) {
      if(single)
        ssl->handshake_circuit_b->do_preproc();
      else
        circuits_preproc_threads.emplace_back(std::thread(&EmpWrapperAG2PC::do_preproc, ssl->handshake_circuit_b));
    }

    for(auto &i : circuits_preproc_threads) {
...
bool ThreePartyHandshake::commit_to_server_certificate(bssl::SSL_HANDSHAKE *hs,
  ...
  auto size = sizeof(Messaging::MessageHeaders) + sizeof(uint64_t) + hash_size;
  ...
  const auto *md = EVP_sha256();
  unsigned out_len{};
  std::array<uint8_t, hash_size> server_key_hash, chts_shts_hash;
  if (!EVP_DigestInit_ex(hash_.get(), md, nullptr) ||
      !EVP_DigestUpdate(hash_.get(), ssl->server_key_share.data(),
                        ssl->server_key_share.size()) ||
      !EVP_DigestFinal_ex(hash_.get(), server_key_hash.data(), &out_len) ||
      out_len != hash_size) {
    return false;
  }
  ...
  if (!EVP_DigestInit_ex(hash_.get(), md, nullptr) ||
      !EVP_DigestUpdate(hash_.get(), ir_chts.data(),
                        ir_chts.size()) ||
      !EVP_DigestUpdate(hash_.get(), ir_shts.data(), ir_shts.size()) ||
      !EVP_DigestFinal_ex(hash_.get(), chts_shts_hash.data(), &out_len) ||
      out_len != hash_size) {
    return false;
  }
  ...
  if (!CBB_init(cbb.get(), size) ||
      ...
      !CBB_add_u64(cbb.get(), hash_size * 2) ||
      !CBB_add_bytes(cbb.get(), reinterpret_cast<const uint8_t *>(chts_shts_hash.data()), sizeof(chts_shts_hash)) ||
      !CBB_add_bytes(cbb.get(), reinterpret_cast<const uint8_t *>(server_key_hash.data()),
...
bool ThreePartyHandshake::derive_traffic_keys(bssl::SSL_HANDSHAKE *hs,
  ...
  std::copy(output.client_iv.begin(), output.client_iv.end(),
            ssl->client_iv.begin());
  std::copy(output.server_iv.begin(), output.server_iv.end(),
            ssl->server_iv.begin());
...
bool ThreePartyHandshake::write_h6(SSL *ssl) {
  if (!ssl || !ssl->verifier) {
    return false;
  }

  auto &hash = ssl->hash_h3h4;
  // We just need to write the hash that produces SCV to the other party.
  // This is passed in to us via the hash span.
  SSL *verifier = ssl->verifier;
  const auto size = sizeof(Messaging::MessageHeaders)*2 + sizeof(hash.size());
  bssl::ScopedCBB cbb;
  ...
  return is_correct_header<Messaging::MessageHeaders::SERVER_CERTIFICATE_PASS>(verifier);
}
...
run_gcm_share_circuit(const EmpWrapperAG2PCConstants::GCMCircuitIn &in,
  ...
  // input:key||xor_mask
  size_t offset = 0;
  // change key_share' bit order
  Util::swap_byte_order(in.key_share.begin(), arr.begin() + offset,
                        in.key_share.size());
  // std::copy(in.key.begin(), in.key.end(), input.begin() + offset);
  offset += sizeof(in.key_share);

  // Util::swap_bitorder(in.xor_mask.begin(), arr.begin() + offset,
  // in.xor_mask.size());
  memcpy(arr.data() + sizeof(in.key_share), in.xor_mask.data(),
         sizeof(in.xor_mask));

  offset += sizeof(in.xor_mask);

  // This uses the fact that out.power_share is the exact right type.
  const bool res = circuit->derive_gcm_shares(arr, out.power_share);
  Util::print_hex_data(out.power_share, "power_share");
  return res;
}
...
make_gcm_share(SSL *const ssl, const std::array<uint8_t, 16> &key_share,
  ...
  // swap xor_mask's bitorder here
  Util::swap_bitorder(in.xor_mask.begin(), in.xor_mask.begin(),
                      in.xor_mask.size());
  EmpWrapperAG2PCConstants::GCMCircuitOut out;
  ...
  // we need to do ((H*B_xor_mask)^A_xor_mask)^A_xor_mask
  if (!is_verifier) {
    for (uint8_t i = 0; i < sizeof(out.power_share); i++) {
      out.power_share[i] ^= in.xor_mask[i];
    }
  }
  ...
  for(unsigned i = 0; i < share.size(); i++) {
    static_assert(sizeof(emp::block) == 16);
    memcpy(block.data(), &share[i], 16);
    Util::swap_bitorder(block.begin(), out_shares.data() + i*16, 16);
  }
...
bool ThreePartyHandshake::derive_gcm_shares(SSL *const ssl) noexcept {
  ...
  return is_correct_header<Messaging::MessageHeaders::GCM_SHARE_DONE>(verifier);
}
